<template>
   <div class="ct05luckyBag" v-if="!!show">
      ...
      <div class="top-section" :style="`background:url(${topSectionBg[cards.length]||''}) no-repeat center top/100%`" v-if="!!cards.length">
          ...
          <div class="phone-num">
            <i></i>
            <input type="text" v-model="phoneNum" placeholder="请输入您的鲸鱼注册账号（手机号）" maxlength="11">
            <!-- <div @click="registerFun">领取</div> -->
          </div>
          <div class="btn"
...
export default {
  data() {
    return {
      ...
      topSectionBg: {
        2: require("./images/top_section_bg_twos1.png"),
        3: require("./images/top_section_bg_threes1.png")
      },
      show: false,
      cards: [],
      phoneNum:"",
      registerShow:false,//判断用户是否注册
    };
  },
  mounted() {
    this.init();
  ...
  computed: {
    ...
    highlight(){
      ...
      let boolTel = reg.test(this.phoneNum)
      return true
      }
  },
  methods: {
    ...
    registerFun(){
      ...
      if(boolTel){
        ...
        this.$http("/activity/common/getUserRegisterInfo",params).then(
          (res)=>{
            // console.log(res)
