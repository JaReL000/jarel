...
service.interceptors.request.use(config => {
  ...
  // Indicator.open('Loading...'); // 全局loading
  return config;
}, error => {
...
service.interceptors.response.use(response => {
  ...
  if (response.data.code === status.SUCCESS) {
    ...
    // console.log(response)
      Flash(response.data.msg)
      return
  }else if (parseInt(response.data.code) === status.NON_PHONE) {
    Flash(response.data.msg)
    setTimeout(function () {
      window.location = response.data.data.redirectUrl
    }, 500);
    ...
    Flash(response.data.msg)
    // return response
    console.log('process.mFormalDomain', process.mFormalDomain)
    if (process.mFormalDomain === location.hostname) {
      ...
      setTimeout(function () {
        window.location = "/index.html#/invited"
      }, 1000);
    }
    // 未绑卡
