...
<div class="index">
  ...
  <div v-show="show">
    ...
    ></tabs>
    <p class="tips">更多理财产品请到鲸鱼APP进行查看</p>
    <!-- 未登陆 -->
...
<script>
  ...
  export default {
    ...
    created(){
      console.log('xxxxxxxxxxxxxxx, 进来了')
    },
    computed: {
      // downloadAPP(){
      //   return downloadAPP
    ...
    methods: {
      ...
      initFun(){
        this.$http({servicename:'api.mvip'}).then((response)=>{
        // this.$http({servicename: 'product.productList',platform_sign:1}).then((response)=>{
       ...
       */
      storage(){
        let combination_id = this.$route.query['combination_id']
