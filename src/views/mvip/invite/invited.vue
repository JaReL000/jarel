  ...
  .form
    ...
    &__item
      ...
      label
        ...
        line-height: rem(30);
        font-size 18px
        background: url('../assets/fa_select_tick.png') no-repeat left center \/ 28px 28px;
      label.isgouZc
        background: url('../assets/fa_select_null.png') no-repeat 6px center \/ 18px 12px, url('../assets/fa_select_tick.png') no-repeat left center \/ 28px 28px;
      .xieyi
        font-size 18px
        color: $mvip-color;
    .aBtnRe
...
<div class="invite">
    ...
    <div class="main">
      <div class="form">
        ...
        <!-- 老用户升级未一级邀请人 -->
        <div class="form__item border-px" v-if="oldUpgrade">
            <input
              type="text"
              class="tel"
              placeholder="请输入邀请码"
        ...
        <div v-if="validated" class="form__item my-sizes">
          <label
              :class="{isgouZc}"
              @click.stop="agreeFun"
          >我同意《</label><a
...
export default {
  ...
  computed: {
    highlight() {
      ...
      let inviteKey = this.inviteKey.length >= 6
      // 第一次
      if (this.validated) {
        // 注册用户
        if (!this.isRegister) {
          return boolTel && boolVar && boolPwd && this.isgouZc && inviteKey;
        } else {
          // 登陆用户
