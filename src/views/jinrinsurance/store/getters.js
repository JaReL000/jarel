const getters = {
  maskShowFlag: state => {
    // 根据弹窗名称是否为空，来控制是否显示遮罩层
    return state.popName !== ''
  },
  gameIsEnd: state => {
    return state.gameOverTimes > 0
  },
  backHeadShowToken: state => {
    return state.appVersion >= 410
  },
  gameEndTimes: state => {
    // 获取结束时间倒计时对象
    ...
    if (!endTime) {
      ...
      return countEndTimer(endTime)
    }
    function countEndTimer (time) {
